...
import { FiMail, FiLock, FiUser, FiPhone, FiMapPin, FiChevronRight, FiCheckCircle, FiArrowLeft } from 'react-icons/fi';
        ...
        <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
          ...
          <div className={`px-8 py-10 text-center transition-colors duration-500 ${role === 'Pujari' ? 'bg-primary-deepBlue' : 'bg-primary-saffron'}`}>
            <h1 className="text-3xl font-bold text-white font-headings">
              {isForgotPassword ? 'Reset Password' : (isLogin ? 'Namaste' : 'Join Us')}
            </h1>
              ...
              <>
                <form onSubmit={handleForgotPassword} className="space-y-4">
                  ...
                  >
                    {isResetLoading ? (
                      ...
                      <>Send Reset Link <FiChevronRight /></>
                    )}
                  </button>
                </form>
                ...
                <div className="mt-8 pt-6 border-t border-gray-100 text-center">
                  <button
                    ...
                    className="text-primary-saffron font-bold hover:underline text-sm flex items-center justify-center gap-1 mx-auto"
                  >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {isLogin && (
                    <div className="text-right">
                      <button
                        type="button"
                        onClick={() => { setIsForgotPassword(true); setError(null); setSuccessMsg(null); setResetEmail(email); }}
                        className="text-sm text-primary-saffron hover:underline font-medium transition-colors"
                      >
                        Forgot Password?
                      </button>
                    </div>
